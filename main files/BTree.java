...
public class BTree<T extends Comparable<T>> {
    ...
    protected Deque<Object> memos = new LinkedList<Object>();
    ...
    public void insert(T value) {
        ...
        if (root == null) {
            ...
            // Push the value to memos without marking any splits
            memos.addFirst(value);
        	}
            ...
            LinkedList<T> splitsMemo = new LinkedList<T>(); // Initialize a list to save raised medians in case of future splits
            while (currentNode != null && !wasAdded) {
            	
            	// If the node has 2t-1 keys then split it
                if (currentNode.getNumberOfKeys() == maxDegree - 1) {
                	
                	// Reach the median value of the current node before splitting it
                	T medianValue = split(currentNode);
                	splitsMemo.addFirst(medianValue);
                    ...
                    currentNode = currentNode.getChild(idx); // Descending down the tree according to the correct indices of the nodes
                }
                
                // Descend the tree and add the key to a leaf
                if (currentNode.isLeaf()) {
                	currentNode.addKey(value); // Reach the exact node, where eventually value will be inserted to
                	wasAdded = true;
                	
                	if (splitsMemo.isEmpty()) // Means no splits was applied
                		memos.addFirst(value);
                	else {
                		splitsMemo.addFirst(value);
                		memos.addFirst(splitsMemo);
                	}
                }
                else {
                    int idx = currentNode.getValuePosition(value);
